...
def main_proccess(E_t1,E_t2,E_t3,E_t4,E_t5,E_t6,S_t1,S_t2,S_t3,S_t4,S_t5,S_t6,P_t,Ybar,f,is_training,num_units,num_layers,dropout):
...
def get_sample_te(dic,mean_last,avg,batch_size_te,time_steps,num_features):
  ...
  #  r1 = np.random.randint(X.shape[0], size=batch_size_te)
    ...
    out[:, 0:4, :] += mean_last.reshape(1,4,WEATHER + SOIL+ PLANTING+ MISC + 1)
    #n=X[r1, :]
    #print(n.shape)
...
def main_program(X, Index,num_units,num_layers,Max_it, learning_rate, batch_size_tr,le,l):
    ...
    with tf.device('/cpu:0'):
        ...
        # bem: modifications  original below.  This was 10, now 11
        #S_t1 = tf.placeholder(shape=[None, 10,1], dtype=tf.float32, name='S_t1')
        S_t1 = tf.placeholder(shape=[None, 11,1], dtype=tf.float32, name='S_t1')

        S_t2 = tf.placeholder(shape=[None, 11,1], dtype=tf.float32, name='S_t2')

        S_t3 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t3')

        S_t4 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t4')

        S_t5 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t5')

        S_t6 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t6')

        # bem S_t7 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t7')

        # bem S_t8 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t8')

        # bem S_t9 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t9')

        # bem S_t10 = tf.placeholder(shape=[None, 11, 1], dtype=tf.float32, name='S_t10')
        ...
        # bem reduced set
        Yhat1,Yhat2= main_proccess(E_t1,E_t2,E_t3,E_t4,E_t5,E_t6,S_t1,S_t2,S_t3,S_t4,S_t5,S_t6,P_t,Ybar,f,is_training,num_units,num_layers,dropout)
        ...
        for i in range(Max_it):
            ...
            PLANTING = 16
            MISC = 3
            out_tr = get_sample(dic, A, avg,batch_size_tr, time_steps=5, num_features=WEATHER + SOIL + PLANTING + MISC + 1)
            ...
            out = out_tr[:, :, 3:-1]
            Batch_X_e = out_tr[:, :, 3:-1].reshape(-1,WEATHER + SOIL + PLANTING)
            ...
            sess.run(train_op, feed_dict={ E_t1: Batch_X_e[:,0:52,:],E_t2: Batch_X_e[:,52*1:2*52,:],E_t3: Batch_X_e[:,52*2:3*52,:],
                                           E_t4: Batch_X_e[:, 52 * 3:4 * 52, :],E_t5: Batch_X_e[:,52*4:5*52,:],E_t6: Batch_X_e[:,52*5:52*6,:], # End of weather readings
                                           S_t1:Batch_X_e[:,312:323,:],S_t2:Batch_X_e[:,323:334,:],S_t3:Batch_X_e[:,334:345,:],
                                           S_t4:Batch_X_e[:,345:356,:],S_t5:Batch_X_e[:,356:367,:],S_t6:Batch_X_e[:,367:378,:], # End of soil readings
                                           # These are extra soil readings?
                                           #S_t7: Batch_X_e[:, 372:382, :], S_t8: Batch_X_e[:, 382:392, :],
                                           #S_t9: Batch_X_e[:, 392:402, :], S_t10: Batch_X_e[:, 402:412, :],
            ...
            if i%1000==0:
                ...
                out_tr = get_sample(dic, A, avg, batch_size=1000, time_steps=5, num_features=WEATHER + SOIL + PLANTING + MISC + 1)
                ...
                rmse_tr,yhat1_tr,loss_tr = sess.run([RMSE,Yhat1,Tloss], feed_dict={ E_t1: Batch_X_e[:,0:52,:],E_t2: Batch_X_e[:,52*1:2*52,:],E_t3: Batch_X_e[:,52*2:3*52,:],
                                           E_t4: Batch_X_e[:, 52 * 3:4 * 52, :],E_t5: Batch_X_e[:,52*4:5*52,:],E_t6: Batch_X_e[:,52*5:52*6,:],
                                           S_t1:Batch_X_e[:,312:323,:],S_t2:Batch_X_e[:,323:334,:],S_t3:Batch_X_e[:,334:345,:],
                                           S_t4:Batch_X_e[:,345:356,:],S_t5:Batch_X_e[:,356:367,:],S_t6:Batch_X_e[:,367:378,:],
                                           # bem commented out
                                           #S_t7: Batch_X_e[:, 372:382, :], S_t8: Batch_X_e[:, 382:392, :],
                                           #S_t9: Batch_X_e[:, 392:402, :], S_t10: Batch_X_e[:, 402:412, :],
                                           P_t: Batch_X_e[:, 378:394, :], #S_t_extra:Batch_X_e[:, 428:, :],
