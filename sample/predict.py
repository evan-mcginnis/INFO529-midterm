...
def conv_res_part_E(E_t,f,is_training,var_name):
    ...
    X = tf.compat.v1.layers.conv1d(E_t, filters=8, kernel_size=9, strides=1, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv00' + var_name, data_format='channels_last', reuse=tf.compat.v1.AUTO_REUSE)

    X = tf.nn.relu(X)
    print('conv1 out E', X)

    X = tf.compat.v1.layers.average_pooling1d(X, pool_size=2, strides=2, data_format='channels_last', name='average_pool')

    X = tf.compat.v1.layers.conv1d(X, filters=12, kernel_size=3, strides=1, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv0' + var_name, data_format='channels_last', reuse=tf.compat.v1.AUTO_REUSE)
    ...
    X = tf.compat.v1.layers.conv1d(X, filters=16, kernel_size=3, strides=1, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv1'+var_name, data_format='channels_last',reuse=tf.compat.v1.AUTO_REUSE)
    ...
    X = tf.compat.v1.layers.conv1d(X, filters=20, kernel_size=3, strides=s0, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv2'+var_name, data_format='channels_last',reuse=tf.compat.v1.AUTO_REUSE)
    X = tf.nn.relu(X)
    X = tf.compat.v1.layers.average_pooling1d(X, pool_size=2, strides=2, data_format='channels_last', name='average_pool')
...
def conv_res_part_S(S_t,f,is_training,var_name):
    ...
    X = tf.compat.v1.layers.conv1d(S_t, filters=4, kernel_size=3, strides=1, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv1'+var_name, data_format='channels_last',reuse=tf.compat.v1.AUTO_REUSE)
    ...
    X = tf.compat.v1.layers.conv1d(X, filters=8, kernel_size=3, strides=1, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv2'+var_name, data_format='channels_last',reuse=tf.compat.v1.AUTO_REUSE)
    ...
    X = tf.compat.v1.layers.conv1d(X, filters=12, kernel_size=2, strides=1, padding='valid',
                         kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), activation=None,
                         name='Conv3'+var_name, data_format='channels_last',reuse=tf.compat.v1.AUTO_REUSE)
...
def main_proccess(E_t1,E_t2,E_t3,E_t4,E_t5,E_t6,S_t1,S_t2,S_t3,S_t4,S_t5,S_t6,S_t7,S_t8,S_t9,S_t10,P_t,Ybar,S_t_extra,f,is_training,num_units,num_layers,dropout):
    ...
    output, _= tf.compat.v1.nn.dynamic_rnn(cell, e_out, dtype=tf.float32)
...
def Cost_function(Y, Yhat):
    ...
    MSE = tf.squeeze(tf.reduce_mean(input_tensor=E2))
    RMSE = tf.pow(MSE, 0.5)
    Loss = tf.compat.v1.losses.huber_loss(Y, Yhat, weights=1.0, delta=5.0)
...
def main_program(X, Index,num_units,num_layers,Max_it, learning_rate, batch_size_tr,le,l):
    ...
    with tf.device('/cpu:0'):
        ...
        E_t1 = tf.compat.v1.placeholder(shape=[None, 52,1], dtype=tf.float32, name='E_t1')

        E_t2 = tf.compat.v1.placeholder(shape=[None, 52, 1], dtype=tf.float32, name='E_t2')

        E_t3 = tf.compat.v1.placeholder(shape=[None, 52, 1], dtype=tf.float32, name='E_t3')
        E_t4 = tf.compat.v1.placeholder(shape=[None, 52, 1], dtype=tf.float32, name='E_t4')
        E_t5 = tf.compat.v1.placeholder(shape=[None, 52, 1], dtype=tf.float32, name='E_t5')
        E_t6 = tf.compat.v1.placeholder(shape=[None, 52, 1], dtype=tf.float32, name='E_t6')



        S_t1 = tf.compat.v1.placeholder(shape=[None, 10,1], dtype=tf.float32, name='S_t1')

        S_t2 = tf.compat.v1.placeholder(shape=[None, 10,1], dtype=tf.float32, name='S_t2')

        S_t3 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t3')

        S_t4 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t4')

        S_t5 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t5')

        S_t6 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t6')

        S_t7 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t7')

        S_t8 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t8')

        S_t9 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t9')

        S_t10 = tf.compat.v1.placeholder(shape=[None, 10, 1], dtype=tf.float32, name='S_t10')

        S_t_extra=tf.compat.v1.placeholder(shape=[None, 4,1], dtype=tf.float32, name='S_t_extra')

        P_t = tf.compat.v1.placeholder(shape=[None, 16, 1], dtype=tf.float32, name='P_t')  #Plant Data




        Ybar=tf.compat.v1.placeholder(shape=[None,5,1], dtype=tf.float32, name='Ybar')


        Y_t = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.float32,name='Y_t')

        Y_t_2 = tf.compat.v1.placeholder(shape=[None, 4], dtype=tf.float32, name='Y_t_2')


        is_training=tf.compat.v1.placeholder(dtype=tf.bool)
        lr=tf.compat.v1.placeholder(shape=[],dtype=tf.float32,name='learning_rate')
        ...
        with tf.compat.v1.name_scope('train'):
            ...
            with tf.control_dependencies(update_ops):
                train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=lr).minimize(Tloss)
